...
function DynamoBackup(options) {
    ...
    this.awsEndpoint = options.awsEndpoint || process.env.AWS_DEFAULT_ENDPOINT;

    this.dynamodb = new AWS.DynamoDB({
        endpoint: this.awsEndpoint,
        accessKeyId: this.awsAccessKey || process.env.AWS_ACCESS_KEY_ID,
        secretAccessKey: this.awsSecretKey || process.env.AWS_SECRET_ACCESS_KEY,
        region: this.awsRegion || process.env.AWS_DEFAULT_REGION
    })
...
DynamoBackup.prototype.backupTable = function (tableName, backupPath, callback) {
    ...
    console.log("+++>" + JSON.stringify({accessKey: self.awsAccessKey, secretKey: self.awsSecretKey, region: self.awsRegion, bucket: self.bucket}));
...
DynamoBackup.prototype._copyTable = function (tableName, itemsReceived, callback) {
    var self = this;
    self.dynamodb.describeTable({ TableName: tableName }, function(err, data) {
        if (err) {
            return callback(err);
        }
